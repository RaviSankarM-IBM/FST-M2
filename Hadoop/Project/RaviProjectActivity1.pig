--Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/ravi/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line: chararray);
inputDialogues5 = LOAD 'hdfs:///user/ravi/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line: chararray);
inputDialogues6 = LOAD 'hdfs:///user/ravi/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line: chararray);
...
rmf hdfs:///user/ravi/outputs;

--Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/ravi/outputs' USING PigStorage('\t');
